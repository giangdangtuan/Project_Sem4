...
public class ApplicationInitConfig {
    ...
    ApplicationRunner applicationRunner(UserRepository userRepository) {
        return args -> {
            ...
            Optional<Role> adminAccountRoleOpt = roleRepository.findByName("ADMIN_ACCOUNT");
            if (adminAccountRoleOpt.isEmpty()) {
                Role adminAccountRole = new Role();
                adminAccountRole.setName("ADMIN_ACCOUNT");
                roleRepository.save(adminAccountRole);
                log.info("Role 'ADMIN_ACCOUNT' has been created.");
            }

            Optional<Role> adminConcertRoleOpt = roleRepository.findByName("ADMIN_CONCERT");
            if (adminConcertRoleOpt.isEmpty()) {
                Role adminConcertRole = new Role();
                adminConcertRole.setName("ADMIN_CONCERT");
                roleRepository.save(adminConcertRole);
                log.info("Role 'ADMIN_CONCERT' has been created.");
            }
