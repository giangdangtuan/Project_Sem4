...
public String GetMapping(HttpServletRequest request, Model model) {
    ...
    if (paymentStatus == 1) {
        ...
        if (selectedSeatIds != null && !selectedSeatIds.isEmpty()) {
            ...
            Concert concert = concertService.getConcertById(seatService.findById(selectedSeatIds.get(0)).getConcert_id());
            bill.setUser_id(userid); // Gán người dùng vào Bill
            bill.setUser(user);
            bill.setConcert_id(concert.getId());
            bill.setConcert(concert);
            for (Integer seatId : selectedSeatIds) {
                Seat seat = seatService.findById(seatId);
                if(seat.isBooked()) {
                    return "user/html/fail.html";
                }
            }
